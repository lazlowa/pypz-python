...
class PipelineSnifferViewer(tk.Tk):
    ...
    def create_from_string(source: str):
        return PipelineSnifferViewer(
            Pipeline.create_from_string(source, mock_nonexistent=True)
        )

    @staticmethod
    def create_from_dto(instance_dto: PipelineInstanceDTO):
        return PipelineSnifferViewer(
            Pipeline.create_from_dto(instance_dto, mock_nonexistent=True)
        )
    ...
    def __init__(self, pipeline: Pipeline):
        ...
        if not isinstance(pipeline, Pipeline):
            raise TypeError(
                f"Invalid argument type: {type(pipeline)}; Expected: {Pipeline}"
            )
        ...
        self._operator_views: dict[Operator, OperatorView] = {}
        self._channel_views: dict[str, ChannelView] = {}
        self._dependency_levels: list[set[Operator]] = order_operators_by_connections(
            pipeline
        )
        self._operator_view_channel_reader_count: dict[Operator, int] = {}
        self._operator_view_channel_writer_count: dict[Operator, int] = {}
        ...
        self._hsb = tk.Scrollbar(
            self._frame, orient="horizontal", command=self._pipeline_canvas.xview
        )
        self._hsb.grid(row=1, column=0, sticky="ew")

        self._vsb = tk.Scrollbar(
            self._frame, orient="vertical", command=self._pipeline_canvas.yview
        )
        self._vsb.grid(row=0, column=1, sticky="ns")

        self._pipeline_canvas.configure(
            xscrollcommand=self._hsb.set, yscrollcommand=self._vsb.set
        )
        ...
        self._sniffer_status = tk.Label(
            self._status_frame, text="Status: Waiting", font=("Helvetica", 12, "bold")
        )
        self._sniffer_status.grid(row=0, column=0, sticky="w")
        self._status_frame.columnconfigure(0, pad=10)
        self._elapsed_time_display = tk.Label(
            self._status_frame,
            text="Elapsed time: 0 [sec]",
    ...
    def __init_sniffer(self):
        ...
        for operator, operator_view in self._operator_views.items():
                ...
                for output_port in input_port.get_connected_ports():
                    if isinstance(output_port, ChannelOutputPort):
                        ...
                        if channel_id not in self._channel_views:
                            ...
                            if (
                                output_port.get_context()
                                not in self._operator_view_channel_writer_count
                            ):
                                self._operator_view_channel_writer_count[
                                    output_port.get_context()
                                ] = 0

                            self._channel_views[channel_id] = ChannelView(
                                self._pipeline_canvas,
                        ...
                        self._pipeline_sniffer.channel_sniffers[channel_id].subscribe(
                            input_port.get_full_name(), channel_reader_view.on_update
                        )
                        input_port_view.channel_views.append(channel_reader_view)
                        self._channel_views[channel_id].channel_reader_views.append(
                            channel_reader_view
                        )
            ...
            for (
                output_port,
                output_port_view,
            ) in operator_view.output_port_views.items():
                for input_port in output_port.get_connected_ports():
                    if isinstance(input_port, ChannelInputPort):
                        ...
                        if channel_id not in self._channel_views:
                            ...
                            if (
                                input_port.get_context()
                                not in self._operator_view_channel_reader_count
                            ):
                                self._operator_view_channel_reader_count[
                                    input_port.get_context()
                                ] = 0
                        ...
                        self._pipeline_sniffer.channel_sniffers[channel_id].subscribe(
                            output_port.get_full_name(), channel_writer_view.on_update
                        )
                        output_port_view.channel_views.append(channel_writer_view)
                        self._channel_views[channel_id].channel_writer_views.append(
                            channel_writer_view
                        )
        ...
        for idx, level in enumerate(self._dependency_levels):
            ...
            for operator in level:
                self._operator_views[operator].draw(
                    ViewConfig.operator_start_pos_x
                    + idx
                    * (ViewConfig.operator_width + ViewConfig.operator_spacing_hor),
                    next_y,
                )
                next_y = (
                    ViewConfig.replica_spacing_ver
                    + self._operator_views[operator].bottom
                )

                for replica in operator.get_replicas():
                    self._operator_views[replica].draw(
                        ViewConfig.operator_start_pos_x
                        + idx
                        * (ViewConfig.operator_width + ViewConfig.operator_spacing_hor),
                        next_y,
                    ...
                    next_y = (
                        ViewConfig.replica_spacing_ver
                        + self._operator_views[replica].bottom
                    )
    ...
    def __sniff(self):
        self._elapsed_time_display.config(
            text=f"Elapsed time: {round(time.time() - self._start_time)} [sec]"
        )
        ...
        if all_finished:
            ...
            self._sniffer_futures = {
                self._executor.submit(channel_sniffer.sniff)
                for channel_sniffer in self._pipeline_sniffer.channel_sniffers.values()
            }
            self.after(1000, self.__sniff)
