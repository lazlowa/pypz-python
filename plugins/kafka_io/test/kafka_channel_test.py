...
class KafkaChannelTest(unittest.TestCase):
    ...
    def test_reader_invoke_open_channel_expect_consumers_and_subscription_init(self):
        ...
        try:
            ...
            self.assertEqual({TopicPartition(KafkaChannelTest.test_channel_name, 0)}, reader._data_consumer.assignment())
            self.assertEqual(reader.get_unique_name() + "." + reader._data_topic_name, reader._data_consumer.config["group_id"])
            self.assertEqual(reader.get_unique_name(), reader._data_consumer.config["client_id"])
            self.assertEqual({TopicPartition(KafkaChannelTest.test_channel_name + WriterStatusTopicNameExtension, 0)}, reader._writer_status_consumer.assignment())
            self.assertEqual(reader.get_unique_name() + "." + reader._writer_status_topic_name, reader._writer_status_consumer.config["group_id"])
    ...
    def test_reader_invoke_open_channel_in_group_for_principal_expect_consumers_and_subscription_init(self):
        ...
        try:
            ...
            self.assertEqual({TopicPartition(KafkaChannelTest.test_channel_name, 0)}, reader._data_consumer.assignment())
            self.assertEqual(reader.get_context().get_group_name() + "." + reader._data_topic_name, reader._data_consumer.config["group_id"])
            self.assertEqual(reader.get_unique_name(), reader._data_consumer.config["client_id"])
            self.assertEqual({
    ...
    def test_reader_invoke_open_channel_in_group_for_non_principal_expect_consumers_and_subscription_init(self):
        ...
        try:
            ...
            self.assertEqual({TopicPartition(KafkaChannelTest.test_channel_name, 1)}, reader._data_consumer.assignment())
            self.assertEqual(reader.get_context().get_group_name() + "." + reader._data_topic_name, reader._data_consumer.config["group_id"])
            self.assertEqual(reader.get_unique_name(), reader._data_consumer.config["client_id"])
            self.assertEqual({
                TopicPartition(KafkaChannelTest.test_channel_name + WriterStatusTopicNameExtension, 0)
    ...
    def test_writer_invoke_open_channel_with_resources_created_expect_all_initialized(self):
        ...
        try:
            ...
            self.assertEqual({TopicPartition(KafkaChannelTest.test_channel_name + ReaderStatusTopicNameExtension, 0)}, writer._reader_status_consumer.assignment())
            self.assertEqual(writer.get_unique_name() + "." + writer._reader_status_topic_name, writer._reader_status_consumer.config["group_id"])
