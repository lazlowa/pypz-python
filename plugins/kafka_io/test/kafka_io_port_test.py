...
class KafkaIOPortTest(unittest.TestCase):
    ...
    def test_almost_everything(self):
        ...
        try:
            ...
            self.assertEqual([{"demoText": f"record_0"}], reader.received_records)
            ...
            self.assertEqual([{"demoText": f"record_0"}, {"demoText": f"record_0"}], reader.received_records)
            self.assertEqual([{"demoText": f"record_1"}], reader_0.received_records)
            ...
            self.assertEqual([{"demoText": f"record_0"}, {"demoText": f"record_0"}], reader.received_records)
            self.assertEqual([{"demoText": f"record_1"}, {"demoText": f"record_1"}], reader_0.received_records)
            reader._on_shutdown()
    ...
    def test_writer_sends_record_to_reader_group_expect_all_reader_got_the_same_record(self):
        ...
        try:
            ...
            self.assertEqual([{"demoText": "from_writer_0_for_all"}],
                             reader.group_input_port.retrieve())
            self.assertEqual([{"demoText": f"from_writer_for_all"}],
                             reader_0.group_input_port.retrieve())
            self.assertEqual([{"demoText": f"from_writer_0_for_all"}],
                             reader_0.group_input_port.retrieve())
