...
class KafkaChannelWriter(ChannelWriter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._data_producer_properties: dict = {}
        """
        Property collection for data producer
        """

        self._writer_status_producer_properties: dict = {}
        """
        Property collection for output status producer
        """

        self._reader_status_consumer_properties: dict = {}
        """
        ...
        self._writer_status_topic_name: str = (
            channel_name + WriterStatusTopicNameExtension
        )
        """
        ...
        self._data_producer_properties["key_serializer"] = lambda key: (
        ...
        self._reader_status_consumer_properties["key_deserializer"] = lambda key: (
            key.decode("utf-8") if key else ""
        )
        self._reader_status_consumer_properties["value_deserializer"] = lambda val: (
            val.decode("utf-8") if val else "Should not be 'None'. Check for cause!"
        )
        ...
        self._reader_status_consumer_properties["group_id"] = (
            self._unique_name + "." + self._reader_status_topic_name
        )
    ...
    def _open_channel(self):
        ...
        try:
            ...
            if (
                (self._data_topic_name not in existing_topics)
                or (self._reader_status_topic_name not in existing_topics)
                or (self._writer_status_topic_name not in existing_topics)
            ):
                return False

            self._target_partition_count = len(
                self._admin_client.describe_topics([self._data_topic_name])[0][
                    "partitions"
                ]
            )
            ...
            if self._reader_status_consumer is None:
                self._reader_status_consumer = KafkaConsumer(
                    **self._reader_status_consumer_properties
                )
                self._reader_status_consumer.subscribe(
                    topics=[self._reader_status_topic_name]
                )
            ...
            if self._writer_status_producer is None:
                ...
                self._writer_status_producer_properties = (
                    self._data_producer_properties.copy()
                )
                self._writer_status_producer_properties["value_serializer"] = (
                    lambda val: val.encode("utf-8")
                )
                self._writer_status_producer = KafkaProducer(
                    **self._writer_status_producer_properties
                )
    ...
    def _write_records(self, records: list[Any]):
        if not isinstance(records, list):
            raise TypeError(
                f"Invalid record type: {type(records)}. List of records (dicts) is expected."
            )
    ...
    def _send_status_message(self, status_string):
        self._writer_status_producer.send(
            self._writer_status_topic_name,
    ...
    def _retrieve_status_messages(self) -> Optional[list]:
        try:
            consumed_state_records = self._reader_status_consumer.poll(
                self._consumer_timeout_ms
            )

            status_list = []

            for _topic, controlRecords in consumed_state_records.items():
...
class KafkaChannelReader(ChannelReader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._data_consumer_properties: dict = {}
        """
        Properties for data consumer as expected by the KafkaConsumer.
        """

        self._writer_status_consumer_properties: dict = {}
        """
        ...
        self._reader_status_producer_properties: dict = {}
        """
        ...
        self._reader_status_topic_name: str = (
            channel_name + ReaderStatusTopicNameExtension
        )
        """
        Input status topic name
        """

        self._consumer_timeout_ms: int = (
            KafkaChannelReader.InitialDataConsumerTimeoutInMs
        )
        """
        This member stores the timeout value for the consumer polling.
        """
        ...
        self._partition_count: int = (
            1 if self._context.is_in_group_mode() else self._context.get_group_size()
        )
        """
        ...
        self._data_consumer_properties["key_deserializer"] = lambda key: (
            key.decode("utf-8") if key else ""
        )
        ...
        self._data_consumer_properties["partition_assignment_strategy"] = [
            RoundRobinPartitionAssignor
        ]

        self._writer_status_consumer_properties["client_id"] = self._unique_name
        self._writer_status_consumer_properties["group_id"] = (
            self._unique_name + "." + self._writer_status_topic_name
        )
        self._writer_status_consumer_properties["key_deserializer"] = lambda key: (
            key.decode("utf-8") if key else ""
        )
        self._writer_status_consumer_properties["value_deserializer"] = lambda val: (
            val.decode("utf-8") if val else "Should not be 'None'. Check for cause!"
        )
        ...
        self._writer_status_consumer_properties["max_partition_fetch_bytes"] = (
            50 * 1024 * 1024
        )
        ...
        self._reader_status_producer_properties["key_serializer"] = lambda key: (
            key.encode("utf-8") if key else None
        )
        self._reader_status_producer_properties["value_serializer"] = lambda key: (
            key.encode("utf-8") if key else "Should not be 'None'. Check for cause!"
        )
    ...
    def _open_channel(self):
        ...
        try:
            ...
            if self._reader_status_producer is None:
                self._reader_status_producer = KafkaProducer(
                    **self._reader_status_producer_properties
                )

            # To ensure idempotence if it would be necessary
            if self._data_consumer is None:
                if (self._context.get_group_name() is not None) and (
                    not self._context.is_in_group_mode()
                ):
                    ...
                    self._data_consumer_properties["group_id"] = (
                        self._unique_name + "." + self._data_topic_name
                    )
                ...
                if not self._context.is_in_group_mode():
                    ...
                    self._target_partition = TopicPartition(
                        self._data_topic_name, self._context.get_group_index()
                    )
                else:
            ...
            if self._writer_status_consumer is None:
                self._writer_status_consumer = KafkaConsumer(
                    self._writer_status_topic_name,
    ...
    def _read_records(self):
        ...
        for _topic, records in consumed_data_records.items():
            for record in records:
    ...
    def _commit_offset(self, offset: int) -> None:
        self._data_consumer.commit(
            {self._target_partition: OffsetAndMetadata(offset, "")}
        )

    def _send_status_message(self, message):
        self._reader_status_producer.send(
            self._reader_status_topic_name,
            key=OutputPortConfigStateControlKey,
            value=message,
        )
    ...
    def _retrieve_status_messages(self) -> Optional[list]:
        try:
            consumed_state_records = self._writer_status_consumer.poll(
                KafkaChannelReader.StatusConsumerTimeoutInMs
            )
            status_list = []
    ...
    def _create_resources(self):
        ...
        try:
            ...
            if self._data_topic_name in existing_topics:
                partitions = self._admin_client.describe_topics(
                    [self._data_topic_name]
                )[0]["partitions"]
                if len(partitions) != self._partition_count:
                    self._admin_client.delete_topics(
                        topics=[self._data_topic_name], timeout_ms=30000
                    )
                    existing_topics = self._admin_client.list_topics()
            ...
            if self._data_topic_name not in existing_topics:
                self._admin_client.create_topics(
                    new_topics=[
                        NewTopic(self._data_topic_name, self._partition_count, 1)
                    ]
                )

            if self._writer_status_topic_name not in existing_topics:
                self._admin_client.create_topics(
                    new_topics=[NewTopic(self._writer_status_topic_name, 1, 1)]
                )

            if self._reader_status_topic_name not in existing_topics:
                self._admin_client.create_topics(
                    new_topics=[NewTopic(self._reader_status_topic_name, 1, 1)]
                )
    ...
    def get_consumer_lag(self) -> int:
        ...
        for topic_partition in end_offsets.keys():
            overall_lag += end_offsets[topic_partition] - self._data_consumer.position(
                topic_partition
            )
