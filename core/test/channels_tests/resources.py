...
from pypz.core.specs.misc import BlankOperator, BlankInputPortPlugin, BlankOutputPortPlugin
...
TEST_DATA_TRANSFER_MEDIUM: dict[str, list | None] = dict()
TEST_CONTROL_TRANSFER_MEDIUM: dict[str, list | None] = dict()
TEST_DATA_OFFSET_STORE: dict[str, int] = dict()
...
class TestChannelWriter(ChannelWriter):
    ...
    def _create_resources(self) -> bool:
        ret_val = self.control_handler(TestChannelWriter._create_resources.__name__, True)
        if ret_val:
            TEST_DATA_TRANSFER_MEDIUM[self._channel_name] = None
            TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + OUTPUT_STATE_POSTFIX] = None
            TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + INPUT_STATE_POSTFIX] = None
        return ret_val

    def _delete_resources(self) -> bool:
        ret_val = self.control_handler(TestChannelWriter._delete_resources.__name__, True)
        if ret_val:
    ...
    def _open_channel(self) -> bool:
        ...
        if ret_val:
            TEST_DATA_TRANSFER_MEDIUM[self._channel_name] = list()
            TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + OUTPUT_STATE_POSTFIX] = list()
            TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + INPUT_STATE_POSTFIX] = list()
        return ret_val
    ...
    def _send_status_message(self, message: str):
        TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + OUTPUT_STATE_POSTFIX].append(message)

    def _retrieve_status_messages(self) -> list:
        retrieved = TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + INPUT_STATE_POSTFIX][self.status_message_offset:]
    ...
    def control_handler(self, method_name: str, default_return_value=None):
        if PARAM_PREFIX_RAISE_MODIFIER + method_name in self.get_configuration():
            raise AttributeError(self.get_configuration()[PARAM_PREFIX_RAISE_MODIFIER + method_name])

        if PARAM_PREFIX_SLEEP_MODIFIER + method_name in self.get_configuration():
            time.sleep(self.get_configuration()[PARAM_PREFIX_SLEEP_MODIFIER + method_name])
...
class TestChannelReader(ChannelReader):
    ...
    def has_records(self) -> bool:
        return 0 < len(TEST_DATA_TRANSFER_MEDIUM[self._channel_name][self.get_read_record_offset():])
    ...
    def _read_records(self) -> list[Any]:
        retrieved = TEST_DATA_TRANSFER_MEDIUM[self._channel_name][self.get_read_record_offset():]
        return retrieved
    ...
    def _open_channel(self) -> bool:
        if (self._channel_name in TEST_DATA_TRANSFER_MEDIUM) and \
           (TEST_DATA_TRANSFER_MEDIUM[self._channel_name] is not None) and \
           self.control_handler(TestChannelReader._open_channel.__name__, True):
            return True
        return False
    ...
    def _send_status_message(self, message: str):
        TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + INPUT_STATE_POSTFIX].append(message)

    def _retrieve_status_messages(self) -> list:
        retrieved = \
            TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + OUTPUT_STATE_POSTFIX][self.output_status_message_offset:]
        self.output_status_message_offset += len(retrieved)

        if 1 < self._context.get_group_size():
            retrieved_input_status = \
                TEST_CONTROL_TRANSFER_MEDIUM[self._channel_name + INPUT_STATE_POSTFIX][self.input_status_message_offset:]  # noqa: E501
