...
from pypz.abstracts.channel_ports import ParamKeyChannelLocationConfig, ParamKeyChannelConfig, \
    ParamKeyPortOpenTimeoutMs, ChannelOutputPort
from .channel_ports_resources import TestPipeline, TestChannelOutputPort, \
...
from core.test.channels_tests.resources import TEST_DATA_TRANSFER_MEDIUM, TEST_CONTROL_TRANSFER_MEDIUM, \
    TEST_DATA_OFFSET_STORE, OUTPUT_STATE_POSTFIX, INPUT_STATE_POSTFIX
...
class ChannelOutputPortTest(unittest.TestCase):
    ...
    def test_channel_output_port_on_resource_creation_expect_initialized_channels_and_created_resources(self):
        pipeline = TestPipeline("pipeline")
        pipeline.writer.output_port.set_parameter(ParamKeyChannelLocationConfig, "local")
        pipeline.writer.output_port.set_parameter(ParamKeyChannelConfig, {
            "return__create_resources": True
        })

        self.assertNotIn(pipeline.reader.input_port_a.get_full_name(), TEST_DATA_TRANSFER_MEDIUM)
        self.assertNotIn(pipeline.reader.input_port_a.get_full_name() + OUTPUT_STATE_POSTFIX, TEST_CONTROL_TRANSFER_MEDIUM)
        self.assertNotIn(pipeline.reader.input_port_a.get_full_name() + INPUT_STATE_POSTFIX, TEST_CONTROL_TRANSFER_MEDIUM)
        self.assertNotIn(pipeline.reader.input_port_b.get_full_name(), TEST_DATA_TRANSFER_MEDIUM)
        self.assertNotIn(pipeline.reader.input_port_b.get_full_name() + OUTPUT_STATE_POSTFIX, TEST_CONTROL_TRANSFER_MEDIUM)
        self.assertNotIn(pipeline.reader.input_port_b.get_full_name() + INPUT_STATE_POSTFIX, TEST_CONTROL_TRANSFER_MEDIUM)

        pipeline.writer.output_port._pre_execution()
        self.assertTrue(pipeline.writer.output_port._on_resource_creation())

        self.assertIsNone(TEST_DATA_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name()])
        self.assertIsNone(TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name() + OUTPUT_STATE_POSTFIX])
        self.assertIsNone(TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name() + INPUT_STATE_POSTFIX])
        self.assertIsNone(TEST_DATA_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name()])
        self.assertIsNone(TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name() + OUTPUT_STATE_POSTFIX])
        self.assertIsNone(TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name() + INPUT_STATE_POSTFIX])
        ...
        for channel_writer in pipeline.writer.output_port._channel_writers:
            ...
            self.assertTrue((pipeline.reader.input_port_a.get_full_name() == channel_writer.get_channel_name()) or
                            (pipeline.reader.input_port_b.get_full_name() == channel_writer.get_channel_name()))
            self.assertEqual("local", channel_writer.get_location())
            self.assertEqual({"return__create_resources": True}, channel_writer.get_configuration())
    ...
    def test_channel_output_port_on_resource_creation_unfinished(self):
        ...
        for channel_writer in pipeline.writer.output_port._channel_writers:
            ...
            self.assertTrue((pipeline.reader.input_port_a.get_full_name() == channel_writer.get_channel_name()) or
                            (pipeline.reader.input_port_b.get_full_name() == channel_writer.get_channel_name()))
            self.assertEqual("local", channel_writer.get_location())
            self.assertEqual({"return__create_resources": False}, channel_writer.get_configuration())
    ...
    def test_channel_output_port_on_resource_creation_in_replica_expect_no_resource_created(self):
    ...
    def test_channel_output_port_on_port_open_expect_opened_ports(self):
        ...
        try:
            ...
            self.assertIsNotNone(TEST_DATA_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name()])
            self.assertIsNotNone(
                TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name() + OUTPUT_STATE_POSTFIX])
            self.assertIsNotNone(
                TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name() + INPUT_STATE_POSTFIX])
            self.assertIsNotNone(TEST_DATA_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name()])
            self.assertIsNotNone(
                TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name() + OUTPUT_STATE_POSTFIX])
            self.assertIsNotNone(
                TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name() + INPUT_STATE_POSTFIX])
        finally:
    ...
    def test_channel_output_port_on_port_close_unfinished(self):
        ...
        self.assertIsNotNone(TEST_DATA_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name()])
        self.assertIsNotNone(TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name() + OUTPUT_STATE_POSTFIX])
        self.assertIsNotNone(TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name() + INPUT_STATE_POSTFIX])
        self.assertIsNotNone(TEST_DATA_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name()])
        self.assertIsNotNone(TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name() + OUTPUT_STATE_POSTFIX])
        self.assertIsNotNone(TEST_CONTROL_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name() + INPUT_STATE_POSTFIX])
    ...
    def test_channel_output_port_send_data(self):
        ...
        try:
            ...
            self.assertEqual(["0", "1", "2"], TEST_DATA_TRANSFER_MEDIUM[pipeline.reader.input_port_a.get_full_name()])
            self.assertEqual(["0", "1", "2"], TEST_DATA_TRANSFER_MEDIUM[pipeline.reader.input_port_b.get_full_name()])
